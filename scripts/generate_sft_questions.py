...
SYSTEM_PROMPT = """你是军事领域 SFT 数据构造专家。当前任务不是回答问题，而是参考一段已评估、已分类的公开文本材料，生成若干个适合构造军事大模型 SFT 数据的问题。最终答案生成阶段只会看到问题本身，不会看到原始材料。因此问题必须像真实用户直接提出的问题，而不是阅读理解题。

生成原则：
1. 问题必须脱离材料也能独立成立，不能出现“根据材料”“结合上文”“文中提到”“从材料看”“上述内容”等指代原文的表达。
2. 问题要贴合给定分类标签，体现该材料最有训练价值的能力方向。
3. 可围绕同一段材料生成多个问题，但每个问题必须关注不同信息点或不同任务能力，避免同义改写凑数。
4. 问题可以是问答、摘要、抽取、对比、解释、结构化整理、非操作性复盘等类型，但要与分类中的 task_types 匹配。
5. 对军事、安全、国防材料保持安全边界：不得生成要求攻击实施、武器制造、规避侦察、突破系统、行动步骤等可操作伤害问题。
6. 如果材料或分类风险较高，应生成安全改写、边界说明、风险识别或拒答类问题。
7. 问题应使用中文，表述自然，尽量对齐真实用户会问的问题。
8. 不要把多个不相干问题硬拼成一个长问题；如需多点回答，应围绕同一个主题。
...
class SFTQuestionGenerator:
    ...
    def _build_user_prompt(self, row: dict[str, Any], question_count: int) -> str:
        ...
        taxonomy = row.get("sft_taxonomy_labels", {})
        ...
        return "\n".join(
            [
                ...
                f"分类标签：{json.dumps(taxonomy, ensure_ascii=False)}",
                "",
                "text：",
                text,
            ]
...
def has_classification(row: dict[str, Any]) -> bool:
    labels = row.get("sft_taxonomy_labels")
    if not isinstance(labels, dict):
        return False
    return bool(labels.get("primary"))
...
def suggested_question_count(row: dict[str, Any], max_questions: int) -> int:
    ...
    secondary: list[Any] = []
    if isinstance(labels, dict):
        raw_task_types = labels.get("task_types")
        raw_secondary = labels.get("secondary")
        if isinstance(raw_task_types, list):
            task_types = raw_task_types
    ...
    if score >= 9 or size >= 1000:
        count += 1
    if len(task_types) >= 4 or len(secondary) >= 2:
        count += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate one or more SFT construction questions for each classified material JSONL row."
    )
    parser.add_argument("--input", required=True, help="Input classified JSONL file, usually under labeled_data.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Also process rows without sft_taxonomy_labels.primary.",
    )
    parser.add_argument(
        "--include-refuse-risk",
        action="store_true",
        help="Also process rows marked refuse_or_exclude. By default they are skipped.",
    )
...
def main() -> None:
    ...
    if not args.include_unclassified:
        rows = [row for row in rows if has_classification(row)]
